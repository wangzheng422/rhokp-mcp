...
```bash
podman pull ghcr.io/wangzheng422/rhokp-mcp:v0.1.2
```

For offline transfer, download the image from the release:

```bash
gh release download v0.1.2 --repo wangzheng422/rhokp-mcp \
  --pattern 'rhokp-mcp-v0.1.2-linux-amd64.tar.gz' --pattern SHA256SUMS
sha256sum --check --ignore-missing SHA256SUMS
gunzip rhokp-mcp-v0.1.2-linux-amd64.tar.gz
podman load -i rhokp-mcp-v0.1.2-linux-amd64.tar
```
