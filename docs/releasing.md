...
3. Make the repository public, then publish a GitHub Release with a tag such as `v0.1.3`, including the workflow in that tagged commit. Creating a tag alone does not trigger this workflow.
...
```bash
MCP_IMAGE=registry.example.com/team/rhokp-mcp:0.1.3
