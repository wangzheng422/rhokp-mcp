...
```bash
podman build --format docker -t localhost/rhokp-mcp:0.1.0 -f Containerfile .
python3 -m unittest discover -s tests -v
```

To transfer a built image without a registry, use `podman save --format docker-archive -o rhokp-mcp.tar localhost/rhokp-mcp:0.1.0` and `podman load -i rhokp-mcp.tar` on the destination. For the rootful host deployment, load with `sudo podman load` into the same rootful image store used by Quadlet. Keep image archives outside the source repository.
...
podman run -d --name rhokp-mcp --read-only --cap-drop=ALL \
  ...
  -p 127.0.0.1:18081:18081 localhost/rhokp-mcp:0.1.0
```
