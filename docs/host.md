...
pull IMAGE_REFERENCE`. For the MCP adapter use the [prebuilt release](../README.md#use-a-prebuilt-release): run `sudo podman pull ghcr.io/wangzheng422/rhokp-mcp:v0.1.0`. The Quadlet template uses that reference. For a downloaded archive use `sudo podman load` instead. Rootful Quadlet uses rootful image storage.

For development or before the first public release, build the adapter locally:

```bash
sudo podman build --format docker -t localhost/rhokp-mcp:0.1.0 -f Containerfile .
```

For a local build, set the MCP Quadlet `Image=` to `localhost/rhokp-mcp:0.1.0` before installing it.
